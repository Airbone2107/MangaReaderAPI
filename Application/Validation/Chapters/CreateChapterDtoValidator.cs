    ...
    public class CreateChapterDtoValidator : AbstractValidator<CreateChapterDto>
    {
        public CreateChapterDtoValidator()
        {
            ...
            RuleFor(p => p.UploadedByUserId) 
                ...
                .When(p => p.UploadedByUserId != 0); // Trong thực tế, UserId sẽ lấy từ context, không phải từ DTO
            ...
            RuleFor(p => p.PublishAt)
                ...
                // .LessThanOrEqualTo(p => p.ReadableAt).WithMessage("Thời điểm xuất bản phải trước hoặc bằng thời điểm có thể đọc."); // Gây lỗi nếu ReadableAt không được set cùng lúc.
